...
const SIdeBar = () => {
  ...
  // search desired user from the user list
  const [searchInput, setsearchInput] = useState(false);
  const desiredUser = searchInput
    ? users.filter((user) =>
...
rounded cursor-pointer max-sm: text-sm ${
          ...
          >
            <img
              ...
              alt=""
              className="w-[34px] aspect-[1/1] rounded-full"
            />
            <div className="flex flex-col leading-5">
              <p>{toTitleCase(user.fullName)}</p>
